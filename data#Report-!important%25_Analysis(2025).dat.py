    def predict(self, input_arr: np.ndarray) -> int:
        ...
        first_layer_output = np.dot(input_arr, self.input_layer_and_first_hidden_layer_weights)
        first_layer_activated = sigmoid(first_layer_output)

        # Compute the second layer output
        second_layer_output = np.dot(first_layer_activated, self.first_hidden_layer_and_second_hidden_layer_weights)
        second_layer_activated = sigmoid(second_layer_output)

        # Compute the final output
        final_layer_output = np.dot(second_layer_activated, self.second_hidden_layer_and_output_layer_weights)
        output = sigmoid(final_layer_output)
